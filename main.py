...
# Workflow.
# Use "screen_reset" for basic requirements 1-2 with UART.
# Use "a4_tape" for basic requirements 3-4 black-tape rectangle vision.
...
# MaixCam Pro: A19(TX) -> controller RX, A18(RX) <- controller TX, common GND.
UART_ENABLED = True
UART_PORT = "/dev/ttyS1"
UART_BAUD = 115200
UART_TX_PIN = "A19"
...
if WORKFLOW == "screen_reset" and MODE == "calibrate":
    ...
    MODE = "track"
if WORKFLOW == "screen_reset":
    init_uart_link()
else:
    uart_status = "VISION ONLY"
...
while True:
    ...
    img.draw_string(4, 112, "FPS:{} {} {} {}".format(fps, WORKFLOW, CAMERA_PROFILE, uart_status), image.COLOR_WHITE)
    disp.show(img)
    time.sleep_ms(1)
